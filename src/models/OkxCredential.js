...
const OkxCredentialSchema = new Schema(
  {
    userID: {
...
OkxCredentialSchema.index({ userID: 1, exchangeID: 1 });

// Virtual 필드: 복호화된 API Key (조회 시에만 사용)
OkxCredentialSchema.virtual("apiKey").get(function () {
  return this.encryptedApiKey ? decryptSecret(this.encryptedApiKey) : null;
});

OkxCredentialSchema.virtual("secretKey").get(function () {
  return this.encryptedSecretKey
    ? decryptSecret(this.encryptedSecretKey)
    : null;
});

OkxCredentialSchema.virtual("passphrase").get(function () {
...
OkxCredentialSchema.set("toJSON", { virtuals: true });

// Static 메서드: API 키 저장 (암호화)
OkxCredentialSchema.statics.createCredential = async function (data) {
  const {
    userID,
    exchangeID,
    apiKey,
...
OkxCredentialSchema.methods.updateCredentials = function (data) {
...
const model = mongoose.model("OkxCredential", OkxCredentialSchema);
